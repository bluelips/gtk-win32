...
$(HARFBUZZ_DLL_FILENAME).lib: $(HARFBUZZ_DLL_FILENAME).dll
$(HARFBUZZ_ICU_DLL_FILENAME).lib: $(HARFBUZZ_ICU_DLL_FILENAME).dll
$(HARFBUZZ_GOBJECT_DLL_FILENAME).lib: $(HARFBUZZ_GOBJECT_DLL_FILENAME).dll
...
$(HARFBUZZ_DLL_FILENAME).dll: config.h $(harfbuzz_dll_OBJS) $(CFG)\$(PLAT)\harfbuzz
	link /DLL $(LDFLAGS) $(HB_DEP_LIBS) /implib:$(HARFBUZZ_DLL_FILENAME).lib -out:$@ @<<
$(harfbuzz_dll_OBJS)
<<
	@-if exist $@.manifest mt /manifest $@.manifest /outputresource:$@;2

$(HARFBUZZ_ICU_DLL_FILENAME).dll: $(HARFBUZZ_DLL_FILENAME).lib $(harfbuzz_icu_OBJS) $(CFG)\$(PLAT)\harfbuzz-icu
	link /DLL $(LDFLAGS) $(HARFBUZZ_DLL_FILENAME).lib $(HB_ICU_DEP_LIBS) /implib:$(HARFBUZZ_ICU_DLL_FILENAME).lib -out:$@ @<<
$(harfbuzz_icu_OBJS)
<<
	@-if exist $@.manifest mt /manifest $@.manifest /outputresource:$@;2

$(HARFBUZZ_GOBJECT_DLL_FILENAME).dll: $(HARFBUZZ_DLL_FILENAME).lib $(harfbuzz_gobject_OBJS) $(CFG)\$(PLAT)\harfbuzz-gobject
	link /DLL $(LDFLAGS) $(HARFBUZZ_DLL_FILENAME).lib $(HB_GOBJECT_DEP_LIBS) /implib:$(HARFBUZZ_GOBJECT_DLL_FILENAME).lib -out:$@ @<<
...
$(CFG)\$(PLAT)\hb-view.exe: $(HARFBUZZ_DLL_FILENAME).lib $(CFG)\$(PLAT)\util $(hb_view_OBJS)
	link $(LDFLAGS) $(HARFBUZZ_DLL_FILENAME).lib $(HB_UTILS_DEP_LIBS) -out:$@ @<<
$(hb_view_OBJS)
<<
	@-if exist $@.manifest mt /manifest $@.manifest /outputresource:$@;1

$(CFG)\$(PLAT)\hb-shape.exe: $(HARFBUZZ_DLL_FILENAME).lib $(CFG)\$(PLAT)\util $(hb_shape_OBJS)
	link $(LDFLAGS) $(HARFBUZZ_DLL_FILENAME).lib $(HB_UTILS_DEP_LIBS) -out:$@ @<<
$(hb_shape_OBJS)
<<
	@-if exist $@.manifest mt /manifest $@.manifest /outputresource:$@;1

$(CFG)\$(PLAT)\hb-ot-shape-closure.exe: $(HARFBUZZ_DLL_FILENAME).lib $(CFG)\$(PLAT)\util $(hb_ot_shape_closure_OBJS)
	link $(LDFLAGS) $(HARFBUZZ_DLL_FILENAME).lib $(HB_UTILS_DEP_LIBS) -out:$@ @<<
